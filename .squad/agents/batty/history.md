...
### 2025-07-22: CI workflow fix and squad state cleanup
- **CI fix**: Updated `.github/workflows/ci.yml` build step from `dotnet build replay.cs` to `dotnet build dotnet-replay.csproj` to match the csproj conversion. Added `dotnet test tests/ReplayTests.csproj` test step.
- **Squad state cleanup**: Removed `.squad/` (24 files) and `.squad-templates/` (34 files) from git tracking on `refactor/csproj-split` branch using `git rm --cached -r`. These are local squad state files that the `squad-main-guard` workflow blocks from merging to main.
- **Preserved**: All `.github/workflows/squad-*.yml` files (11 workflow files) remain tracked — they're legitimate CI/CD workflows, not squad state.
- **Key decision**: Squad state files should never be committed to branches intended for merge. The guard workflow enforces this, but cleaning them proactively avoids merge friction.
